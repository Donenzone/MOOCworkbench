...
from ExperimentsManager.models import Experiment, ChosenExperimentSteps
from ExperimentsManager.models import ExperimentStep
...
class ExperimentTestCase(TestCase):
    ...
    def test_create_new_experiment_get(self):
        response = self.client.get(reverse('new_experiment'))
        self.assertEqual(response.status_code, 302)

    def test_create_new_experiment_get_not_signed_in(self):
        c = Client()
        response = c.get(reverse('new_experiment'))
        self.assertEqual(response.status_code, 302)

    @patch('ExperimentsManager.views.get_user_repositories')
    ...
    @patch('ExperimentsManager.views.create_new_github_repository_local')
    def test_create_new_experiment_post(self, mock_create_new_github_repo):
        ...
        response = self.client.post(reverse('new_experiment'), data=data)

        self.assertEqual(response.status_code, 302)
        self.assertEqual(response.url, reverse('choose_experiment_steps', kwargs={'experiment_id': 2}))
    ...
    def test_choose_experiment_steps_get(self):
        response = self.client.get(reverse('choose_experiment_steps', kwargs={'experiment_id': 1}))
    ...
    def test_choose_experiment_steps_post(self):
        ...
        response = self.client.post(reverse('choose_experiment_steps', kwargs={'experiment_id': 1}), data=data)
        self.assertEqual(response.status_code, 200)
    ...
    def test_choose_experiment_steps_double_post(self):
        ...
        response = self.client.post(reverse('choose_experiment_steps', kwargs={'experiment_id': 1}), data=new_data)
        self.assertEqual(response.status_code, 200)
        chosen_experiment_steps = ChosenExperimentSteps.objects.filter(experiment=self.experiment)
