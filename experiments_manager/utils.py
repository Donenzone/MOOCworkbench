...
from experiments_manager.consumers import send_experiment_creation_status_update
...
def init_git_repo_for_experiment(experiment, cookiecutter):
    ...
    send_experiment_creation_status_update(username, ExperimentProgress.STEP_CREATING_GITHUB_REPO)
    ...
    send_experiment_creation_status_update(username, ExperimentProgress.STEP_CREATING_BOILERPLATE_CODE)

    git_helper.commit('Cookiecutter template added')
    git_helper.push()
    send_experiment_creation_status_update(username, ExperimentProgress.STEP_PUSHING_BOILERPLATE_CODE)

    requirements_file = github_helper.view_file('requirements.txt')
    parse_requirements_file(experiment, requirements_file)
    send_experiment_creation_status_update(username, ExperimentProgress.STEP_READING_DEPENDENCIES)
