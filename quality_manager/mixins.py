...
class MeasurementMixin(object):
    ...
    def _get_recent_measurements_for_all_types(self, step):
        ...
        for measure in measures:
            recent_result = self._get_most_recent_measurement(step, measure)
            if recent_result:
                measurement_list.append(recent_result)
        return measurement_list

    def get_recent_measurements_for_type(self, step, measure):
        recent_results = ExperimentMeasureResult.objects.filter(
                                                                step=step,
                                                                measurement=measure).order_by('created')[:21]
        return recent_results

    def _get_most_recent_measurement(self, step, experiment_measure):
